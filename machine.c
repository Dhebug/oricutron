...
void atmoswrite(struct m6502 *cpu, unsigned short addr, unsigned char data)
{
  ...
  else if((!oric->romdis) && (addr >= 0xc000)) return;       // Can't write to ROM!
...
void shut_machine(struct machine *oric)
{
  if(oric->drivetype == DRV_MICRODISC)
  {
    microdisc_free(&oric->md);
    oric->drivetype = DRV_NONE;
  }
  if(oric->drivetype == DRV_BD500)
  {
    bd500_free(&oric->bd);
    oric->drivetype = DRV_NONE;
  }
  if(oric->drivetype == DRV_JASMIN)
  {
    jasmin_free(&oric->jasmin);
    oric->drivetype = DRV_NONE;
  }
  if(oric->drivetype == DRV_PRAVETZ)
  {
    pravetz_free(&oric->pravetz);
    oric->drivetype = DRV_NONE;
  }
  if(oric->mem)
  {
